...
src/
  ...
  utils/         날짜(KST), ID, 인사 문구, 루틴 아이콘·색상 유틸리티
```
...
- **루틴 카드** (`rounded-[20px]`, `RoutineCard` 컴포넌트): 오늘 요일에 해당하는 루틴만 필터
- 상태 pill: 완료됨(초록) / 건너뜀(회색) / N일 연속(오렌지) / 미체크(회색)
- 루틴 제목 키워드 기반 **아이콘 자동 매핑** + ID 기반 **색상 팔레트** 자동 적용
- **스와이프 제스처**: 우→ 완료, 좌→ 건너뜀 (임계값 80px), 방향별 힌트 레이어
- **선택 모드**: 멀티 선택 체크박스 토글 (`isSelectMode`)
- 완료 시: 초록 배경(`bg-emerald-50/40`), 인라인 메모 입력 후 저장/건너뛰기, 완료 메모 좌측 보더 액센트
  - 미완료 시: "완료" / 메모 아이콘 / 건너뜀 아이콘 버튼 그리드
...
### 10. 웹뷰 (`/webview`) — `WebViewPage`

이용약관, 개인정보처리방침 등 외부 URL을 앱 내 인앱 브라우저로 표시.

**UI 요소**
- **헤더**: 뒤로가기 화살표 + 쿼리 파라미터 `?title=` 값을 제목으로 표시
- **iframe**: `?url=` 파라미터 URL 렌더링, `sandbox="allow-same-origin allow-scripts allow-popups"`
- URL 파라미터 없으면 이전 화면으로 즉시 이동
...
| `DaySelector` | 요일 선택 원형 버튼 그리드 (`w-11 h-11 rounded-full`) | 루틴 생성, 루틴 편집 |
...
| `RoutineCard` | 스와이프 제스처 + 선택 모드 지원 루틴 카드 | 홈 |
| `RoutineNotFound` | 루틴을 찾을 수 없을 때 표시하는 빈 상태 UI | 루틴 상세, 루틴 편집 |
...
### `utils/greeting.ts` — 동기부여 인사 문구

시간대(hour), 진행률(progress), 최고 스트릭(topStreak), 총 루틴 수(totalCount)를 받아 상황별 문구를 반환.

| 조건 | 반환 문구 |
|------|----------|
| 루틴 없음 | "첫 루틴을 만들어 보세요!" |
| 진행률 100% | "오늘 루틴을 모두 완료했어요!" |
| 진행률 ≥ 50% | "절반 이상 해냈어요, 조금만 더!" |
| 오전 (< 12시) | "좋은 아침이에요, 오늘도 힘내봐요!" |
| 오후 (12~18시) | "오후도 파이팅! 루틴을 이어가요." |
| 스트릭 ≥ 7일 | "N일 연속 대단해요, 계속 가볼까요?" |
| 기본 | "오늘도 꾸준함이 답이에요." |

### `utils/routineColor.ts` — 아이콘·색상 자동 매핑

- **`getRoutineIcon(title, id)`**: 루틴 제목의 키워드(예: "운동", "독서", "명상" 등 30+ 카테고리)를 순서대로 매칭해 Material Symbols 아이콘명 반환. 매칭 없으면 ID 해시 기반 폴백 아이콘 반환.
- **`getRoutineColor(id)`**: ID 해시 기반으로 emerald / blue / violet 3종 색상 팔레트 중 하나를 반환. `{ accent, bgTint, iconBg, iconText, progressBar }` 필드로 구성.
